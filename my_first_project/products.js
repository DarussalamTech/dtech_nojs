...
db = new Db('dtech_node_db', server);
//Open Connection
db.open(function(err, db) {
    if (!err) {
        console.log("Connected to 'dtech_node_db' database");
        db.collection('products', {strict: true}, function(err, collection) {
            if (err) {
                console.log("The 'dtech_node_db' collection doesn't exist.Creating it with sample data…");
// populateDB();
            }
        });
    }
